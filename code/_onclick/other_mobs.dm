...
/mob/living/carbon/human/RangedAttack(var/atom/A, var/params)
	...
	if((istype(A, /turf/simulated/floor) || istype(A, /turf/unsimulated/floor) || istype(A, /obj/structure/lattice) || istype(A, /obj/structure/catwalk)) && isturf(loc) && shadow && !is_physically_disabled()) //Climbing through openspace
		...
		if(T.Adjacent(shadow) && above.CanZPass(src, UP)) //Certain structures will block passage from below, others not
			...
			visible_message("<span class='notice'>[src] inizia ad arrampicarsi su \the [A]!</span>", "<span class='notice'>Inizi ad arrampicarti su \the [A]!</span>")
			shadow.visible_message("<span class='notice'>[shadow] starts climbing onto \the [A]!</span>")
			if(do_after(src, 50, A))
				visible_message("<span class='notice'>[src] si arrampica su \the [A]!</span>", "<span class='notice'>Ti arrampichi su \the [A]!</span>")
				shadow.visible_message("<span class='notice'>[shadow] si arrampica su \the [A]!</span>")
				src.Move(T)
			else
				visible_message("<span class='warning'>[src] si arrende di tentare di arrampicarsi su \the [A]!</span>", "<span class='warning'>Ti arrendi di cercare di arrampicarti su \the [A]!</span>")
				shadow.visible_message("<span class='warning'>[shadow] si arrende a cercare di arrampicarsi su \the [A]!</span>")
			return TRUE
...
/mob/living/carbon/alien/UnarmedAttack(var/atom/A, var/proximity)
	...
	A.attack_generic(src,rand(5,6),"morso")
...
/mob/living/carbon/slime/UnarmedAttack(var/atom/A, var/proximity)
	...
	var/mob/living/M = A
	if(!istype(M))
		A.attack_generic(src, (is_adult ? rand(20,40) : rand(5,25)), "acciuffato") // Basic attack.
	else
		var/power = max(0, min(10, (powerlevel + rand(0, 3))))

		switch(src.a_intent)
			if (I_HELP) // We just poke the other
				M.visible_message("<span class='notice'>[src] ti tocca con delicatezza [M]!</span>", "<span class='notice'>[src] ti tocca con gentilezza!!</span>")
			if (I_DISARM) // We stun the target, with the intention to feed
				...
				if(prob(stunprob))
					...
					M.visible_message("<span class='danger'>[src] è saltato su [M]!</span>", "<span class='danger'>[src] è saltato sopra di te!</span>")
					M.Weaken(power)
				else
					M.visible_message("<span class='danger'>[src] ha cercato di saltare sopra [M]!</span>", "<span class='danger'>[src] ha cercato di saltare sopra di te!</span>")
				M.updatehealth()
			if (I_GRAB) // We feed
				Wrap(M)
			if (I_HURT) // Attacking
				if(iscarbon(M) && prob(15))
					M.visible_message("<span class='danger'>[src] è saltato sopra di [M]!</span>", "<span class='danger'>[src] ti è salito addosso!</span>")
					M.Weaken(power)
				else
...
/mob/living/simple_animal/UnarmedAttack(var/atom/A, var/proximity)
	...
	if(istype(A,/mob/living))
		if(melee_damage_upper == 0)
			...
			return
		if(ckey)
			admin_attack_log(src, A, "Ha [attacktext] la sua vittima.", "E' stato [attacktext] dal suo aggressore.", attacktext)
	setClickCooldown(DEFAULT_ATTACK_COOLDOWN)
